...
from pointcept.models.peft import SceneAwareStructuralAdapter
...
class Block(PointModule):
    def __init__(
        ...
        channels,
        num_heads,
        ssa_hidden_channels=64,
        enable_ssa=False,
        num_serialization_orders=1,
        patch_size=48,
        mlp_ratio=4.0,
        qkv_bias=True,
        qk_scale=None,
        ...
        self.base_order_index = order_index
        self.num_serialization_orders = num_serialization_orders
        ...
        self.ssa = (
            SceneAwareStructuralAdapter(channels, ssa_hidden_channels)
            if enable_ssa
            else None
        )

    def _set_serialization_partition(self, partition_index):
        """Select one of the valid serialization-induced partitions."""
        order_index = (
            self.base_order_index + int(partition_index)
        ) % self.num_serialization_orders
        self.attn.order_index = order_index
    ...
    def forward(self, point: Point):
        ...
        if self.ssa is not None:
            shortcut = shortcut + self.ssa(point)
        if self.pre_norm:
...
@MODELS.register_module("PTV3-PointPiT")
class PointTransformerV3(PointModule):
    def __init__(
            ...
            for i in range(enc_depths[s]):
                enc.add(
                    Block(
                        ...
                        ssa_hidden_channels=ssa_hidden_channels,
                        enable_ssa=True,
                        num_serialization_orders=len(self.order),
        ...
        # Generic model initialization touches the zero-initialized SSA output
        # projections.  Restore the adapters' identity-at-start behavior.
        for module in self.modules():
            if isinstance(module, SceneAwareStructuralAdapter):
                module.reset_parameters()

    def set_serialization_partition(self, partition_index):
        """Switch every attention block to a consistent serialization view."""
        for module in self.modules():
            if isinstance(module, Block):
                module._set_serialization_partition(partition_index)
