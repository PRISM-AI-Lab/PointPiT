...
@MODELS.register_module()
class FinetuneSegmentorV2(nn.Module):
    def __init__(
        self,
        num_classes,
        backbone_out_channels,
        ...
        self.criteria = build_criteria(criteria)
        self.freeze_backbone = freeze_backbone
        self.activate_param = activate_param
        if self.freeze_backbone:
            if self.activate_param is not None:
                for name, p in self.backbone.named_parameters():
                    p.requires_grad = any(
                        active_param in name for active_param in self.activate_param
                    )
